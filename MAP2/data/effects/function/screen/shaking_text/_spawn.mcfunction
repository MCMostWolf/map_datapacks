$summon minecraft:text_display ~ ~ ~ {Tags:["effects.load","effects.entity","effects.shaking_text"],\
...
$execute as @n[distance=..0.01,type=minecraft:text_display,tag=effects.load] run function effects:screen/shaking_text/load {time:$(time)}
#音效模拟
