$summon minecraft:text_display ~ ~ ~ {Tags:["effects.load","effects.entity","effects.moving_text"],\
...
$execute as @n[distance=..0.01,type=minecraft:text_display,tag=effects.load] run function effects:screen/moving_text/load {time:$(time)}
#音效模拟
